...
public class SimpleSequenceManager : Singleton<SimpleSequenceManager>
{
    [Header("任务三基础设置")]
    ...
    [Header("Json存储相关设置")]
    public string SimpleSequenceTask = "challengeTaskTwo";
    ...
    public void OnButtonClicked(int number)
    {
        ...
        if (number == numberSequence[currentInputIndex])
        {
            // 正确选择，改变当前按钮材质
